...
Waifu2x::eWaifu2xError Waifu2x::waifu2x(double factor, const void* source, void* dest, int width, int height, int in_channel, int in_stride, int out_channel, int out_stride)
{
	...
	if (output_depth != 32) // 出力深度は32bitだけ
		return eWaifu2xError_InvalidParameter;
	...
	// Matへ変換
	{
		cv::Mat original_image(cv::Size(width, height), CV_MAKETYPE(CV_32F, in_channel), (void *)source, in_stride);
	...
	if (out_channel == 1)
		cv::cvtColor(write_iamge, write_iamge, cv::COLOR_RGB2GRAY);

	// 出力配列へ書き込み
	{
		const auto width = write_iamge.size().width;
		const auto stride = write_iamge.step[0];
