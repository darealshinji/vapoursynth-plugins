...
static void DrawDigit( uint8_t *dstp, int dst_pitch, int x, int y, int num )
{
...
//    const Pixel32 textColor = 0x00FF00;
    ...
    for( ptrdiff_t tx = 0; tx < 10; tx++ )
    {
        for( ptrdiff_t ty = 0; ty < 20; ty++ )
        {
            uint8_t *dp = &dstp[(x + tx) + (y + ty) * dst_pitch];
...
static void DrawString( VSFrameRef *dst, int x, int y, const char *s, const VSAPI *vsapi )
{
    uint8_t *dstp      = vsapi->getWritePtr( dst, 0 );
    int      dst_pitch = vsapi->getStride  ( dst, 0 );
    for( int xx = 0; *s; ++s, ++xx )
    {
        DrawDigit( dstp, dst_pitch, x + xx, y, *s - ' ' );
    }
}
