...
static inline void histogramAdd_avx2(const uint16_t * _x, uint16_t * _y, const uint16_t bins) noexcept {
    for (uint16_t i = 0; i < bins; i += 16) {
        const Vec16us x = Vec16us().load_a(_x + i);
        const Vec16us y = Vec16us().load_a(_y + i);
        (y + x).store_a(_y + i);
    }
}

static inline void histogramSub_avx2(const uint16_t * _x, uint16_t * _y, const uint16_t bins) noexcept {
    for (uint16_t i = 0; i < bins; i += 16) {
        const Vec16us x = Vec16us().load_a(_x + i);
        const Vec16us y = Vec16us().load_a(_y + i);
        (y - x).store_a(_y + i);
...
static inline void histogramMulAdd_avx2(const uint16_t a, const uint16_t * _x, uint16_t * _y, const uint16_t bins) noexcept {
    for (uint16_t i = 0; i < bins; i += 16) {
        const Vec16us x = Vec16us().load_a(_x + i);
        const Vec16us y = Vec16us().load_a(_y + i);
        (y + a * x).store_a(_y + i);
...
void process_avx2(const T * srcp, T * VS_RESTRICT dstp, uint16_t * VS_RESTRICT hCoarse, uint16_t * VS_RESTRICT hFine, const CTMFData * d,
    ...
    for (int i = 0; i < height; i++) {
        ...
        if (padLeft)
            histogramMulAdd_avx2(d->radius, &hCoarse[0], H.coarse, bins);
        for (int j = 0; j < (padLeft ? d->radius : 2 * d->radius); j++)
            histogramAdd_avx2(&hCoarse[bins * j], H.coarse, bins);
        for (int k = 0; k < bins; k++)
            histogramMulAdd_avx2(2 * d->radius + 1, &hFine[bins * width * k], &H.fine[k][0], bins);

        for (int j = padLeft ? 0 : d->radius; j < (padRight ? width : width - d->radius); j++) {
            uint16_t sum = 0, * segment;
            int k, b;

            histogramAdd_avx2(&hCoarse[bins * std::min(j + d->radius, width - 1)], H.coarse, bins);
            ...
            if (luc[k] <= j - d->radius) {
                ...
                for (luc[k] = j - d->radius; luc[k] < std::min(j + d->radius + 1, width); luc[k]++)
                    histogramAdd_avx2(&hFine[bins * (width * k + luc[k])], H.fine[k], bins);
                if (luc[k] < j + d->radius + 1) {
                    histogramMulAdd_avx2(j + d->radius + 1 - width, &hFine[bins * (width * k + width - 1)], &H.fine[k][0], bins);
                    luc[k] = j + d->radius + 1;
                }
            } else {
                for (; luc[k] < j + d->radius + 1; luc[k]++) {
                    histogramSub_avx2(&hFine[bins * (width * k + std::max(luc[k] - 2 * d->radius - 1, 0))], H.fine[k], bins);
                    histogramAdd_avx2(&hFine[bins * (width * k + std::min(static_cast<int>(luc[k]), width - 1))], H.fine[k], bins);
                }
            }

            histogramSub_avx2(&hCoarse[bins * std::max(j - d->radius, 0)], H.coarse, bins);
...
template void process_avx2<uint8_t, 16>(const uint8_t *, uint8_t *, uint16_t *, uint16_t *, const CTMFData *, const int, const int, const int, const bool, const bool) noexcept;
template void process_avx2<uint16_t, 32>(const uint16_t *, uint16_t *, uint16_t *, uint16_t *, const CTMFData *, const int, const int, const int, const bool, const bool) noexcept;
template void process_avx2<uint16_t, 64>(const uint16_t *, uint16_t *, uint16_t *, uint16_t *, const CTMFData *, const int, const int, const int, const bool, const bool) noexcept;
template void process_avx2<uint16_t, 128>(const uint16_t *, uint16_t *, uint16_t *, uint16_t *, const CTMFData *, const int, const int, const int, const bool, const bool) noexcept;
template void process_avx2<uint16_t, 256>(const uint16_t *, uint16_t *, uint16_t *, uint16_t *, const CTMFData *, const int, const int, const int, const bool, const bool) noexcept;
...
void processRadius2_avx2(const VSFrameRef * src, VSFrameRef * dst, const int plane, const uint8_t widthPad, const VSAPI * vsapi) noexcept {
...
template void processRadius2_avx2<uint8_t, Vec32uc, 32>(const VSFrameRef *, VSFrameRef *, const int, const uint8_t, const VSAPI *) noexcept;
template void processRadius2_avx2<uint16_t, Vec16us, 16>(const VSFrameRef *, VSFrameRef *, const int, const uint8_t, const VSAPI *) noexcept;
#endif
