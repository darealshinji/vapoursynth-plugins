...
# define AVS_LinkCallChangeThis(arg, x)  !AVS_linkage || offsetof(AVS_Linkage, arg) >= AVS_linkage->Size ?     0 : (x->*(AVS_linkage->arg))
...
public:
  ...
  PVideoFrame(const volatile PVideoFrame& x) AVS_BakedCode( AVS_LinkCall(PVideoFrame_CONSTRUCTOR1)(const_cast<const PVideoFrame&>(x)) )
  ...
  void operator=(const volatile PVideoFrame& x) AVS_BakedCode( AVS_LinkCall(PVideoFrame_OPERATOR_ASSIGN1)(const_cast<const PVideoFrame&>(x)) )
  void operator=(const volatile PVideoFrame& x) volatile AVS_BakedCode( AVS_LinkCallChangeThis(PVideoFrame_OPERATOR_ASSIGN1, const_cast<PVideoFrame*>(this))(const_cast<const PVideoFrame&>(x)) )
